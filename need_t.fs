...
: need-test-new
    ...
    \ Try new with region target.
    s" 0 r1100 1 1 2" string-to-stack           \ tkn targ ned-typ act-id dom-id
    need-new                                    \ ned

    cr ." need 2: " dup .need cr

    \ Test.
    dup need-get-dom-inst-id 2 <> abort" domain inst id ne 2?"
    dup need-get-act-inst-id 1 <> abort" action inst id ne 1?"
    dup need-get-target is-region? invert abort" target not a region?"
    dup need-get-info 0<> abort" action info s/b zero?"
