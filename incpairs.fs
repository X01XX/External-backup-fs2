...
: inc-pairs-priority-non-adjacent-pairs ( reg-lst0 -- pr-lst t | f )
    \ Check arg.
    assert( tos is-region-list? )

    \ Make a list of all non-adjacent pairs.
    list-new over                           \ reg-lst0 nadj-prs' reg-lst0

    foreach                                 \ reg-lst0 nadj-prs' reg-lnk
        dup link-get-data                   \ reg-lst0 nadj-prs' reg-lnk regx
        region-states-adjacent?             \ reg-lst0 nadj-prs' reg-lnk bool
        ifnot
            dup link-get-data               \ reg-lst0 nadj-prs' reg-lnk regx
            #2 pick                         \ reg-lst0 nadj-prs' reg-lnk regx nadj-prs'
            list-push-struct                \ reg-lst0 nadj-prs' reg-lnk
        then
    next
                                            \ reg-lst0 nadj-prs'
    \ Check if there are no non-adjacent pairs.
    dup list-is-empty?                      \ reg-lst0 nadj-prs' bool
    if
        list-deallocate                     \ reg-lst0
        drop
        false
        exit
    then
    ...
    dup list-get-length 1 =                 \ reg-lst0 nadj-prs' bool
    if
        nip                                 \ nadj-prs'
        true
        exit
    ...
    \ Make a list of all adjacent pairs.
    swap                                    \ nadj-prs' reg-lst0
    list-new swap                           \ nadj-prs' adj-prs' reg-lst0

    foreach                                 \ nadj-prs' adj-prs' reg-lnk
        dup link-get-data                   \ nadj-prs' adj-prs' reg-lnk regx
        region-states-adjacent?             \ nadj-prs' adj-prs' reg-lnk bool
        if
            dup link-get-data               \ nadj-prs' adj-prs' reg-lnk regx
            #2 pick                         \ nadj-prs' adj-prs' reg-lnk regx adj-prs'
            list-push-struct                \ nadj-prs' adj-prs' reg-lnk
        then
    next
    ...
    swap region-list-deallocate             \ nadj-prs' pri-regs'
